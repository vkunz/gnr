...
GNRMaterialLibrary::GNRMaterialLibrary()
{
...
void GNRMaterialLibrary::import(istream& is)
{
	m_is = &is;
	while (m_is->good())
	{
		...
		m_data.insert(pair<string, GNRMaterial>(m_matname, *m_material));
		delete m_material;
	}
}

bool GNRMaterialLibrary::getName()
{
...
bool GNRMaterialLibrary::getData()
{
	bool gotData = true;
	m_buf.clear();
	if (m_is->good())
...
void GNRMaterialLibrary::parseData()
{
	if (m_buf.size() < 2)
		return;
