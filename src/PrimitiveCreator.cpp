 ...
 */
void PrimitiveCreator::createCuboid(const Vertex& position, const Vertex& angles, const Vertex& dimension, const std::string& matname)
{
...
void PrimitiveCreator::createSphere(float radius, const std::string& matname)
{
	//create main primitive container
	...
	//set specific tags on assembly
	m_primitive->setType(IS_PRIMITIVE);
	m_primitive->setPrimitiveType(SPHERE);
	m_primitive->setMaterial(matname);

	//set dimension of vertex
	Vertex dim(2*radius, 2*radius, 2*radius);
	m_primitive->dimension() = dim;
